...
- **Reagendar deja registro (decisión aprobada):** cuando un paciente cambia su cita, la cita original **no se edita ni se borra**, se marca con estado `cancelada` y se crea una nueva. Misma lógica que el historial de versiones de dietas: nada se sobreescribe, todo deja rastro. Beneficio adicional: queda visible cuántas veces ha reagendado un paciente, información útil para la nutrióloga. **Pantalla de gestión de citas, IMPLEMENTADA:** ver, reagendar, cancelar y marcar asistencia desde el expediente, con confirmación obligatoria antes de ejecutar las 4 acciones que cambian estado (cancelar, reagendar, marcar completada, no asistió), agregada el 1 sep 2026
...
- **Sincronización con Google Calendar, IMPLEMENTADA Y VERIFICADA (1 sep 2026).** Dirección Sistema → Google (la dirección inversa, Google → Sistema, queda pendiente y diferida, no bloqueante)
  - **Dos calendarios independientes de Marifer**, cada uno con su propio correo: `maferul2309@gmail.com` (consulta privada) y `nutricion.unido@gmail.com` (pacientes de UniDO, correo compartido que maneja personal de UniDO sin acceso a la app)
  - **Campo nuevo `origen_consulta` en el paciente** (Privado/UniDO, radio buttons en alta y edición), decide a cuál de los dos calendarios va cada cita
  - **Autorización vía OAuth de cada cuenta personal**, no cuentas de servicio (ver hallazgo importante en la Fase 5, sección de respaldo a Drive — el mismo problema de cuota aplicaría aquí). Cliente OAuth reutilizado del que ya existe para el respaldo de Drive, dos tokens nuevos e independientes
  - **Casilla "Esta cita ya está en Google Calendar (no duplicar)"** en los formularios de agendar y reagendar: resuelve el caso real de que Marifer agende directo en su celular, o que UniDO agende y ella solo registre después en la app. Sin tildar por default
  - Reagendar borra el evento anterior y crea uno nuevo; cancelar borra el evento. Un fallo de Google nunca bloquea la operación en el sistema (se guarda la cita, se avisa en el log si el calendario no se pudo sincronizar)
...
- **Registro de opciones ya prescritas, IMPLEMENTADO Y VERIFICADO (1 sep 2026).** Al aprobar una dieta (nunca en un borrador), cada opción de su menú se guarda en `opciones_prescritas`. Al generar una dieta nueva, se le pasan a Gemini las opciones de la última dieta **aprobada** más los comentarios cualitativos del follow-up más reciente (qué le gustó, qué no, cambios que hizo, en qué puede mejorar, ajustes acordados) — como contexto informativo, no como instrucción de repetir ni de evitar. Decisión explícita: un paciente puede pedir seguir con la misma dieta ajustando solo porciones, o cambiarla de fondo por dificultad o disgusto; Gemini decide con criterio, igual que con el resto del expediente
- **Lectura del InBody, IMPLEMENTADA Y VERIFICADA (1 sep 2026).** Se sube la imagen del reporte (no PDF: el sistema InBody real envía por correo un `.jpg`, se ajustó sobre la marcha) a `gemini-3.7-flash` (multimodal, confirmado vigente) con prompt restrictivo, en JSON estricto. Verificado con un reporte real de InBody370S de UniDO
  - **Campos extraídos**, confirmados exactos contra un reporte real: peso, IMC, porcentaje de grasa, masa grasa (kg), MME, grasa visceral, tasa metabólica basal. Insumo interno, nunca mostrado al paciente
  - **Historial dentro del propio reporte:** el prompt pide todos los puntos de la sección "Historial de Composición Corporal" del reporte, con su fecha; los puntos antiguos normalmente solo traen peso/MME/% grasa, el resto se deja en null para esos puntos sin inventar valores
  - Manejo de errores: reintento automático si el JSON falla, chequeo de sensatez (peso entre 20 y 300 kg, descarta solo ese campo si está fuera de rango)
  - **Identificación del paciente, simplificada por decisión de producto:** en vez de la búsqueda global de 3 capas originalmente especificada, la función vive **dentro del expediente ya abierto** de un paciente (arrastrar y soltar), así que la identidad ya se conoce de antemano. Se conserva una red de seguridad de 3 verificaciones antes de guardar, para el error real que sí puede pasar (soltar el archivo en el expediente equivocado): (1) alerta si el ID de InBody ya está ligado a **otro** paciente, (2) comparación de edad contra `fecha_nacimiento`, (3) comparación de sexo y estatura contra lo ya registrado. Ninguna bloquea por completo, todas avisan y dejan decidir a Marifer
  - **Campo nuevo `estatura` en el paciente**, se llena solo la primera vez que se lee un InBody de ese paciente (nunca hay formulario manual para capturarla; los pacientes de consulta privada, que nunca usan InBody, simplemente no la tienen, sin pérdida de funcionalidad)
  - La imagen original **se descarta tras la extracción**, nunca se guarda en el servidor (ahorro de almacenamiento, decisión explícita)
...
- [x] **Respaldo diario cifrado a Google Drive, IMPLEMENTADO Y VERIFICADO (1 sep 2026).** Empaqueta `sistema_nutricion.db` y `.env`, cifra con GPG (AES256 simétrico, frase guardada fuera del servidor), sube con `rclone`, y rota respaldos con más de 30 días. Programado con `systemd timer` a las 3:00 AM diario. Verificado con una restauración de prueba completa (descarga, descifrado, y confirmación del contenido)
  - **Giro de arquitectura a medio camino:** el plan original usaba una cuenta de servicio de Google (sin login interactivo), pero se descubrió que las cuentas de servicio no tienen cuota de almacenamiento en un Drive personal (`storageQuotaExceeded`) — esa función es exclusiva de Google Workspace. Se cambió a autorización OAuth de la cuenta personal de Marifer, con token de renovación automática (requiere que el proyecto de Google esté en estado "Production", no "Testing", para que el token no expire cada 7 días)
  - **Alcance del permiso:** por una limitación de `rclone authorize`, el permiso otorgado terminó siendo de Drive completo en vez del más acotado `drive.file` originalmente buscado; decisión consciente de no repetir la autorización solo por eso
  - **Arquitectura reutilizable:** el mismo mecanismo OAuth (proyecto de Google Cloud, cliente OAuth "Desktop app") se va a usar para la futura sincronización de los 2 calendarios de Google de Marifer (Fase 1/6), no fue trabajo de un solo uso
